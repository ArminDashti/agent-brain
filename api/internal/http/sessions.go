...
func (h *Handler) ListSessions(c *gin.Context) {
	...
	sqlStr := `
		SELECT uuid, name, subtitle, status, unified_mode, workspace_id, workspace_path,
...
func (h *Handler) GetSession(c *gin.Context) {
	...
	_ = h.db.QueryRowContext(c.Request.Context(), `
		SELECT COUNT(*), COALESCE(SUM(CASE WHEN has_thinking = 1 THEN 1 ELSE 0 END), 0)
...
func (h *Handler) GetSessionThinking(c *gin.Context) {
	...
	rows, err := h.db.QueryContext(c.Request.Context(), `
		...
		FROM turns
		WHERE session_uuid = ? AND has_thinking = 1
...
func scanSessionSummary(row scannable) (gin.H, error) {
	var (
		...
		extractedAt, updatedAt                           sql.NullString
	)
	...
	return gin.H{
		...
		"extracted_at":          nullStringTime(extractedAt),
		"updated_at":            nullStringTime(updatedAt),
	}, nil
}
...
func nullStringTime(v sql.NullString) any {
	if !v.Valid || v.String == "" {
		return nil
	}
